        ...
        return (
            <div className="fixed w-full inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 overflow-y-auto">
                <div onClick={onPrev} className='flex transition-transform transform hover:scale-110 cursor-pointer w-2/5 text-center hover:bg-opacity-40 h-svh justify-center items-center'>
                    <button
                        onClick={onPrev}
                        className="text-purple-400 p-2 focus:outline-none hover:text-purple-600"
                    >
                        <svg
                            fill="none"
                            stroke="currentColor"
                ...
                <div className="bg-gray-100 border z-10 h-svh pt-6 p-8 rounded-lg max-w-3xl w-4/5 my-10 relative overflow-y-auto max-h-screen text-black">
                    <button
                        onClick={onClose}
                        className="absolute top-2 right-2 text-purple-400 rounded-full p-2 hover:text-red-600 transition-transform transform hover:scale-110"
                    >
                        &times;
                    </button>
                    <h2 className="text-3xl font-bold mb-4 text-purple-400">{project.title}</h2>
                    <div className="flex flex-col md:flex-row gap-6">
                        <div className="relative transition-transform transform hover:scale-110">
                            <span className="top-2 left-2 z-10 bg-purple-500 bg-opacity-75 text-black p-1 rounded">Laptop View</span>
                            <Image
                                src={project.image}
                                width={400}
                                height={500}
                        ...
                        <div className="relative mx-auto md:mx-0 transition-transform transform hover:scale-110">
                            <span className="top-2 left-2 z-10 bg-purple-500 bg-opacity-75 text-black p-1 rounded">Mobile View</span>
                            <Image
                    ...
                    {project.features && (
                        <div>
                            <h3 className="text-xl text-purple-400 font-semibold mb-2">Key Features:</h3>
                    ...
                    <div className="flex mt-4 justify-between items-center text-center">
                        <button
                            onClick={onClose}
                            className="bg-purple-500 text-black py-2 px-4 rounded hover:bg-purple-600 transition-transform transform hover:scale-110 mb-2 sm:mb-0"
                        >
                            Close
                        </button>
                        <Link href={project.link} passHref>
                            <p
                                ...
                                className="text-purple-400 dark:text-black hover:text-purple-600 inline-flex items-center transition-transform transform hover:scale-110"
                            >
                                Visit Site
                                <svg
                                    fill="none"
